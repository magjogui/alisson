...
###############################################################
######################Rutinas agregadas a ultima Hora #####################
###############################################################

#Insertar nuevos comandos,listar y procesar comandos,Encriptar
#Falta que probarlas pero creo que deben de fun.. ciao tactac

sub insertar_comandos()
{
		conectar();
		print"Por favor digite el nombre del comando\n";
		$comando=<STDIN>;
		chop $comando;
		...
		$valido ="select * from comandos where id_comando='$comando'";
		$sth = $dbh->prepare($valido);
		$sth->execute();
		@valida=$sth->fetchrow_array;
		if(@valida)
		{
			print $error;
		}
			...
			print"Por favor digite la descripcion del comando\n";
			$descripcion=<STDIN>;
			chop $descripcion;
	
			print"Por favor digite la sintaxis del comando\n";
			$sintaxis=<STDIN>;
			chop $sintaxis;

			print"Por favor digite la version del IOS en que es soportado este comando\n";
			$ios=<STDIN>;
			chop $ios;
	
	$listado ="insert into comandos values('NULL','$comando','$descripcion','$sintaxis','$ios')";
		$sth = $dbh->prepare($listado);
		$sth->execute();
		print "Registros Agregados Satisfactoriamente";
		   }		
...
{		
		conectar();
		$val ="select nombre,descripcion,sintaxis,ios_router from comandos";
		$sth = $dbh->prepare($val);
		$sth->execute();
		print "Lista de comandos Adicionados\n";
		while(@row=$sth->fetchrow_array()) 
		{
			print "  @row\n";
		}
			print "Seleccione el comando a ejecutar:\n";
			$id_comando=<STDIN>;
			chop $id_comando;
			consola($dat,$id_comando);
}
################ fin de la funcion 

#CREATE TABLE `comandos` (
#  `id_comando` int(10) NOT NULL auto_increment,
#  `nombre` varchar(25) NOT NULL default '',
#  `descripcion` varchar(50) NOT NULL default '',
#  `sintaxis` varchar(50) NOT NULL default '',
#  `ios_router` varchar(10) NOT NULL default '',
#  KEY `id_comando` (`id_comando`)
#) ENGINE=MyISAM DEFAULT CHARSET=latin1;

sub ocultando()
{
$retorna=crypt($datos, "$f!a$c!il");
return($retorna);
}